...
image::images/code-block.png[]

[TIP]
Source blocks are modified by link:ui/src/js/06-code.js[]
+
The source is highlighted using highlight.js, the colours are defined in link:ui/src/css/highlight.css.
...
The script will find all HTML `div`'s with a class of `tabs`, then iterate through the next sibling while the element contains a code block until it reaches one without.
